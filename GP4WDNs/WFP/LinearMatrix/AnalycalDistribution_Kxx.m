...
% pipe covariance
CoeffVariance
% Demand covariance and its index
DemandVariance
DemandIndex
...
%% Construct pesudo demands  The Vech method;
...
% no need to plus the elevation of tanks, because we are calculating
% covariance, the constant valvue doesn't make any sense
Kbb(TankHeadIndex,:) = TankErrorVariance;

inverseofAA = (A'*A)\eye(NumberofX_A,NumberofX_A);
Kxx =  inverseofAA * A'*diag(Kbb)*A*inverseofAA;

% % tic
% [AnalysisMatrix,B] = Construct_Variance_A_b_Pipe(NumberofX,A,demand_MC,coefficient_MC,TankMeasurement_MC,K_c,ForConstructA);
% % toc
Covar = Kxx;%cov(AnalysisSolution');
