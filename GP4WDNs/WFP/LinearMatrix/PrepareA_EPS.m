function [d, IndexInVar, InitialParameter, ForConstructA, ForConstructb, Variable_Symbol_Table, Solution, MassEnergyMatrix4GP, Inverse] = PrepareA_EPS(inpname, TestCase)
MC_times = 1000;
% demand uncertainty, 10% around estimated demand value
multiple_demand = 0.2;
% pipe uncertainty, 5% around estimated demand value
multiple_pipe_coefficient = 0.2;
...
while (tstep > 0)
    ...
    PumpSpeed = [PumpSpeed;LinkSettings(:, PumpIndex);];
    PipeRoughness = [PipeRoughness;LinkSettings(:, PipeIndex)];
...
TankElevation = Elevation(NodeTankIndex);
...
DemandVariance = (multiple_demand.*Demand_known./2.576).^2;
CoeffVariance = (multiple_pipe_coefficient.*PipeRoughness./2.576).^2;
TankErrorVariance = (accuracy.*(TankHead-TankElevation)./2.576).^2;
% [Coefficient_MC, CoeffVariance] = GenerateMCCoefficient(PipeRoughness, MC_times, multiple_pipe_coefficient);
% 
% TankInitialLevel = d.getNodeTankInitialLevel;
% TankInitialLevel = TankInitialLevel(NodeTankIndex);
...
Inverse = struct('DemandVariance', DemandVariance, ... 
