function PlotConfidienceInterval4SelectedHead(Data,Data_sd,ConfidenceLevel1,ConfidenceLevel2,SelectedHead,Variable_Symbol_Table)
...
[m,n] = size(SelectedHead);
varIndex = [];
% find the index of CaredElements
for  i = 1:n
    var_index = find(strcmp(Variable_Symbol_Table,SelectedHead{i}))
...
plot1=plot(x_ax, Datax,'DisplayName','Expectation','LineWidth',2,'Color','green',...
        ...
        'Marker','diamond');
xlim([1 24])
ylim([min(Y_min2) max(Y_min2)])
% lable
ylabel(SelectedHead{1},'FontSize',fontsize+3,'interpreter','latex');
...
for id = 2:n
    ...
    plot1 = plot(x_ax, Datax,'DisplayName','Expectation','LineWidth',2,'Color','green',...
        ...
        'Marker','diamond');
    % lable
    ylabel(SelectedHead{id},'FontSize',fontsize+3,'interpreter','latex');
    xlim([1 24])
    ylim([min(Y_min1) max(Y_max1)+40])
    set(subplot1,'FontSize',fontsize,'TickLabelInterpreter','latex');
    set(gca,'xtick',[])
    set(gca,'xticklabel',[])
    labley= floor((min(Y_min2)+ max(Y_min2))*0.5);
    set(subplot1,'FontSize',fontsize,'TickLabelInterpreter','latex','YTick',...
            [labley],'YTickLabel',{string(labley)});
    if(id==n)
        ylim([min(Y_min2) max(Y_min2)])
...
set(gcf,'PaperUnits','inches','PaperPosition',[0 0 24 18])
print(figure1,['ConfidentLevel','Head'],'-depsc2','-r300');