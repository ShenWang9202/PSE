...
for i = 1:NumberofX
    TempCellIndex = cell(NumberofX,3);
    AllCount = 0;
    for j = i:NumberofX
        ...
        testMap = containers.Map('KeyType','double','ValueType','double');
        for m = 1:CountMatrix_i
            for n = 1:CountMatrix_j
                ...
                CurrentValue = ValueMatrix_i(m)*ValueMatrix_j(n);
         
                if(~testMap.isKey(ind))
                    testMap(ind) = CurrentValue;
                else
                    testMap(ind) =testMap(ind)+ CurrentValue;
                end
                %AnalysisMatrix(index,ind) = (AnalysisMatrix(index,ind)) + ValueMatrix_i(m)*ValueMatrix_j(n);
            end
        end
        %columnKey = cell2mat(testMap.keys);
        %valueT = cell2mat(testMap.values);
        AllCount = AllCount + testMap.Count;
        TempCellIndex{index,1} = index*ones(1,testMap.Count);
        TempCellIndex{index,2} = cell2mat(testMap.keys);
        TempCellIndex{index,3} = cell2mat(testMap.values);
...
%     for keysdf = 1:testMap.Count
%     AnalysisMatrix(index,columnKey(keysdf)) = valueT(keysdf);
%     end
end
